...
- **`Model '<path>' not found in MTEB registry`:** Expected when a local folder is looked up in the registry. Use `--model-path /path/to/ckpt` (and `--hub-id` for Qwen3/Harrier). A local directory passed to `--model` is fine; it should no longer crash on registry lookup.
